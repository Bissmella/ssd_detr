...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, output_dir):
        ...
        for imagename in imagenames:
            self.recs[imagename] = parse_rec(self._anno_file_template.format(imagename))
    ...
    def process(self, inputs, outputs, classname):
        ...
        scores = outputs['pred_logits'].to(self._cpu_device)

        classes = torch.empty_like(scores[:, :, 0:1]).to(self._cpu_device)
        #class_list = [classname]
        #TODO Fill each row of the new tensor with the corresponding value from the list
        for i, value in enumerate(classname):
            classes[i, :, :] = classname[i]

        for input, boxes, scores, classes in zip(inputs, boxes, scores, classes):
            image_id = input["image_id"]
            #instances = output.to(self._cpu_device)

            boxes = boxes.numpy()
            scores = scores.tolist()
            classes = classes.tolist()   #should be similar shape to scores/boxes filled with selected class
    ...
    def evaluate(self):
        """
        #selected_class: List of lenght batch_size of the class selected for each image in the batch as prompt
        Returns:
        ...
        self._logger.info(
            "Evaluating pascal voc using pascal voc metric. "
            ...
            for cls_id, cls_name in enumerate(self._class_names):
                ...
                for thresh in range(50, 100, 5):
                    rec, prec, ap = voc_eval(
                        ...
                        use_07_metric= False#self._is_2007,
                    )
...
def voc_eval(
    ...
    #Ground Truth
    # get out the extra classes from the target
    ...
    #Detection
    # read dets GIVE 3 THINGS:  imgids, confidence, BB
    ...
    tp = np.zeros(nd)
    fp = np.zeros(nd)
    for d in range(nd):   # for object query of each prediction  
